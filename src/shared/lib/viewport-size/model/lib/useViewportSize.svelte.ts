 ...
 *
 * Uses {@link ResizeObserver} on `document.documentElement` so updates fire only
 * when the layout viewport actually changes size (resize, orientation change) —
 * not on every scroll frame. That keeps position consumers from recomputing
